...
public class AcidWashingRecipeGen extends ProcessingRecipeGen {
    ...
    GeneratedRecipe ETHYLENE_TO_PLASTIC = convert(
            GTMaterialItems.MATERIAL_ITEMS.get(TagPrefix.plate, GTMaterials.Polyethylene).get(),
            GTItems.PLASTIC_BOARD.asItem());
    GeneratedRecipe PC_TO_PLASTIC = convert(
            GTMaterialItems.MATERIAL_ITEMS.get(TagPrefix.plate, GTMaterials.PolyvinylChloride).get(),
            GTItems.PLASTIC_BOARD.asStack(2));
    GeneratedRecipe PFE_TO_PLASTIC = convert(
            GTMaterialItems.MATERIAL_ITEMS.get(TagPrefix.plate, GTMaterials.Polytetrafluoroethylene).get(),
            GTItems.PLASTIC_BOARD.asStack(4));
    GeneratedRecipe PBI_TO_PLASTIC = convert(
            GTMaterialItems.MATERIAL_ITEMS.get(TagPrefix.plate, GTMaterials.Polybenzimidazole).get(),
            GTItems.PLASTIC_BOARD.asStack(8));
    GeneratedRecipe EPOXY_TO_BOARD = convert(
            GTMaterialItems.MATERIAL_ITEMS.get(TagPrefix.plate, GTMaterials.Epoxy).get(),
            GTItems.EPOXY_BOARD.asStack());
    GeneratedRecipe REINFORCED_EPOXY_TO_BOARD = convert(
            GTMaterialItems.MATERIAL_ITEMS.get(TagPrefix.plate, GTMaterials.ReinforcedEpoxyResin).get(),
            GTItems.FIBER_BOARD.asStack());
    GeneratedRecipe TOLUENE_TO_TNT = convert(GTItems.GELLED_TOLUENE.asStack(4), Items.TNT.getDefaultInstance());
    GeneratedRecipe IRON_BLOCK = convert(AllBlocks.WEATHERED_IRON_BLOCK.asItem(),
