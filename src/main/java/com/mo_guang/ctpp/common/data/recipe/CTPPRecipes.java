...
//                ROTOR, "C", AllItems.PRECISION_MECHANISM, "G", GLASS, "E", AllBlocks.SHAFT);
        ...
        registerMachineRecipe(provider, true, CTPPMachines.KINETIC_OUTPUT_BOX , "H", "S", 'S',
                AllBlocks.SHAFT.asStack(), 'H', HULL);
        registerMachineRecipe(provider, false, "kinetic_input_box_to_output_", CTPPMachines.KINETIC_INPUT_BOX, "d", "H", 'H', CTPPMachines.KINETIC_OUTPUT_BOX);
        registerMachineRecipe(provider, false, "kinetic_output_box_to_input_", CTPPMachines.KINETIC_OUTPUT_BOX, "d", "H", 'H', CTPPMachines.KINETIC_INPUT_BOX);

        VanillaRecipeHelper.addShapedRecipe(provider,  CTPP.id("mechanical_upgrade_bus"), CTPPMachines.MECHANICAL_UPGRADE_BUS.asStack(),
                "ABA",
                "DCD",
                "ABA",
    ...
    public static void registerMachineRecipe(Consumer<FinishedRecipe> provider,
        ...
        for (MachineDefinition machine : machines) {
            ...
            if (machine != null) {
                ...
                VanillaRecipeHelper.addShapedRecipe(provider, setMaterialInfoData, CTPP.id(prefix + machine.getName()), machine.asStack(),
                        prepRecipe);
            }
        }
    }
